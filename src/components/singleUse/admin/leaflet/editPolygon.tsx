...
const EditPolygon = ({ parkData, addressData }: { parkData: localsResponse[]; addressData: addressResponse[] }) => {
        ...
        <div key={index}>
          {value.polygon.coordinates.map((positions, key) => (
            <Polygon key={key} positions={positions} color={"#8FBC94"}>
              {!drawingContext && (
                <Popup>
                  <PolygonEditForm polygonID={value.id} parkData={value} addressData={addressData} />
                </Popup>
              )}
        ...
        <Select
          ...
          onBlur={(value) => {
            ...
            if (parseInt(value.target.value) != -1) {
              const temp = parkData
                .find((individualData) => individualData.id == parseInt(value.target.value))
